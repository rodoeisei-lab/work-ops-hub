...
- `std_value = null` は選択可能。画面の「STDを手入力する」で当日の確認値を入れて計算する
- マスタ未登録の物質は、名称を直接入力してから「STDを手入力する」で計算できる
- 繰り返し使う未登録物質は、画面の「一覧にない物質」から端末内の候補として保存できる（共有マスタは変更しない）
...
- 入力途中の内容は `gc-calculator-state-v4` キーへ自動保存
- ページ再訪時に復元
- 「入力を消す」で確認ダイアログ後に消去
- 端末内で追加した物質は `gc-calculator-custom-materials-v1` に別保存する
